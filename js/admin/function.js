...
function CreateTitleSEO(){
	...
	name_vn = f.name_vn.value.replace(/\"|_/g,"”");
	name_en = f.name_en.value.replace(/\"|_/g,"”");
	
	f.title_vn.value = f.name_vn.value = name_vn;
	f.title_en.value = f.name_en.value = name_en;
	
	f.keyword_vn.value = name_vn.toLowerCase() + ", " + StripVi(name_vn).toLowerCase();
	f.keyword_en.value = name_en.toLowerCase();

	f.unique_key_vn.value = StripVi2(name_vn).toLowerCase();
	f.unique_key_en.value = StripVi2(name_en).toLowerCase();
	
	f.des_vn.value = f.short_vn.value.replace(/\"|_/g,"”");
	f.des_en.value = f.short_en.value.replace(/\"|_/g,"”");
...
function CreateTitleSEOWidthTag(){
	...
	if (f.tags.value) {
		vl = f.tags.value.replace(/\"|_/g,"”");
		f.keyword_vn.value = f.keyword_en.value = vl.replace(/\,|_/g,", ");
	} else {
		f.keyword_vn.value = name_vn.toLowerCase() + ", " + StripVi(name_vn).toLowerCase();
		f.keyword_en.value = name_en.toLowerCase();
	}
