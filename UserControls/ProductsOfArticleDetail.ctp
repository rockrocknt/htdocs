<? 
	global $FullUrl, $lg, $cat1, $cat_link_in_article_detail;
	 
	if(!$products_of_article) $products_of_article = $random_products;
	
	if($products_of_article) {
		$cat_link_in_article_detail = GetCategoryProduct($products_of_article[0]);
?>

<div class="dt-pro">
  <h3 class="h3-title"><? if($random_products){ ?>Sản phẩm ngẩu nhiên<? }else{ ?><a href="<?=$cat_link_in_article_detail?>" title="<?=$cat["name_$lg"]?>"><?=$cat["name_$lg"]?></a><? } ?></h3>
  <div class="product">
    <? 
  	 $j = 0;
  ...
  ?>
    ...
    <? }?>
    <? if(!$random_products){ ?><div class="cl"></div>
    <div class="view-all"><a href="<?=$cat_link_in_article_detail?>" title="<?=$cat["name_$lg"]?>">Xem tất cả sản phẩm <?=strtolower($cat["name_$lg"])?></a></div><? } ?>
  </div>
</div>
<? } ?>
