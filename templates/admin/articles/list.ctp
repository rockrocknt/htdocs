...
<div class="control_frm" style="margin-top:0">
  	<div style="float:left;">
        ...
        <input type="button" class="blueB" value="Hiện" onclick="ChangeAction('admin.php?do=articles&act=show<?=isset($_GET['cid'])?'&cid='.$_GET['cid']:''?><?=isset($_GET['page'])?'&page='.$_GET['page']:''?>');return false;" />
        <input type="button" class="blueB" value="Ẩn" onclick="ChangeAction('admin.php?do=articles&act=hide<?=isset($_GET['cid'])?'&cid='.$_GET['cid']:''?><?=isset($_GET['page'])?'&page='.$_GET['page']:''?>');return false;" />
        ...
        <input type="button" class="blueB" value="Xuất Excel" onclick="location.href='admin.php?do=articles&act=export'" />
    </div>
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    ...
    <? 
      ...
      <tr>
        ...
        <td align="center">
            <input type="text" value="<?=$stips[$i]["num"]?>" name="ordering[]" onkeypress="return OnlyNumber(event)" class="tipS smallText" original-title="Số thứ tự tin tức, xếp từ nhỏ đến lớn" id="number<?=$stips[$i]['id']?>" onchange="return updateNumber('articles', '<?=$stips[$i]['id']?>')" />
        ...
        <td class="title_name_data">
            <a href="admin.php?do=articles&act=edit&id=<?=$stips[$i]["id"]?><?=isset($_GET['cid'])?'&cid='.$_GET['cid']:''?><?=isset($_GET['page'])?'&page='.$_GET['page']:''?>" class="tipS SC_bold"><?=$stips[$i]["name_vn"]?>
        ...
        <td align="center">
            <a href="admin.php?do=articles&act=change_active<?=isset($_GET['cid'])?'&cid='.$_GET['cid']:''?>&id=<?=$stips[$i]["id"]?>&current=<?=$stips[$i]["active"]?><?=isset($_GET['page'])?"&page=".$_GET['page']:''?>" title="" class="smallButton tipS" original-title="<?=$stips[$i]['active']?'Click để ẩn':'Click để hiện'?>"><img src="./images/admin/icons/color/<?=$stips[$i]['active']?'tick':'hide'?>.png" alt=""></a>
        </td>
        <td class="actBtns">
