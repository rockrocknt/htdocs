...
//    $catobjj = new Categories(Categories::getCatByID($catList[$iaa]['category_id']));
    $catId = $catList[$iaa]['category_id'];
    $catobjj = new Categories(Categories::getCatByID($catId));
    $catlink = $catobjj->getLink();
    ?>
    <section class="products-grid products-holder ">
        <div class="row-fluid">
            <div class="module title3 heading-line">
                <a href="<?=$catlink?>" style="float:right;" class="link-2 xemthemlink">
                    <i class="uk-icon-location-arrow"></i>&nbsp;Xem thêm </a>
                <h4 onclick="redirect('<?=$catlink?>');" class="module-title homeCat">
                    <span><?= $catobjj->getName() ?></span></h4>
            </div>
        </div>
        <div class="row-fluid">
            <?php
            ...
            $where = " `cid` = " . $catId;
            $sql = "select  *   from products  where  ( " . $where . " and name_$lg<>'' and active=1) order by id desc limit 3";
